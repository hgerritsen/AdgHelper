...
* "copy advice sheets from sharepoint.R" does just that. It is a bid fiddly to get the url that lists the file names on the advice sharepoint but if you follow the instructions it should work - Note: it only works for the old ICES sharepoint. The alterative is to manually copy the files to a local folder.
